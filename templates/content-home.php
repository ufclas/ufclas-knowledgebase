<?php
	...
	if ( !is_wp_error($terms) && $terms ):
		...
		foreach ( $terms as $term ):
			...
			$article_count_label = ( $post_count == 1 )? __('Article', 'ufclas-knowledgebase') : __('Articles', 'ufclas-knowledgebase');
			...
			if ( $post_count > $max_posts ){
				printf('<div class="kbe-view-all"><a href="%s" title="%s">%s</a></div>', 
					$term_link,
					__('View articles in ', 'ufclas-knowledgebase') . $term_label,
					__('View All Articles', 'ufclas-knowledgebase')
				);
			}
		...
		echo '<p>' . __('No Knowledge Base Categories found.', 'ufclas-knowledgebase') . '</p>';
	endif;
?>